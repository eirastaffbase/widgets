...
import { CHANNELS, SHOW_DETAILS, INITIAL_ISSUES } from "./constants.js";
import { TODAY, DATE_RANGE, dateKey, isSameDay } from "./utils.js";
import { fetchScheduleFromStaffbase } from "./api.js";
...
export default function BroadcastOpsWidget() {
  ...
  if (loading) return (
    <div className="min-h-screen w-full flex items-center justify-center" style={{ background: "#f5f3ee" }}>
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: "#f5f3ee", fontFamily: "'Source Sans Pro', -apple-system, system-ui, sans-serif" }}>
      <style>{`
      ...
      <header className="sticky top-0 z-40 border-b" style={{ background: "#1a2744", borderColor: "#0f1a30" }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs tracking-widest uppercase" style={{ color: "#a8b4cc" }}>Programming & Delivery Hub</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center rounded-md p-1" style={{ background: "rgba(255,255,255,0.08)" }}>
              {[{ val: "station", label: "Station", Icon: User }, { val: "hq", label: "HQ Staff", Icon: Users }].map(({ val, label, Icon }) => (
                <button key={val} onClick={() => setRole(val)} className="flex items-center gap-2 px-3 py-1.5 rounded text-xs font-semibold transition-all" style={{ background: role === val ? "#f5a623" : "transparent", color: role === val ? "#1a2744" : "#a8b4cc" }}>
        ...
        <nav className="max-w-7xl mx-auto px-6 flex gap-1 overflow-x-auto scrollbar-thin">
          {TABS.map(({ id, label, icon: Icon, badge }) => {
            ...
            return (
              <button key={id} onClick={() => setActiveTab(id)} className="flex items-center gap-2 px-4 py-3 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap" style={{ borderColor: active ? "#f5a623" : "transparent", color: active ? "white" : "#a8b4cc" }}>
      ...
      {/* ── Notification panel ── */}
      {showNotifPanel && (
        <div className="fixed top-20 right-6 z-50 w-96 rounded-lg shadow-2xl border fade-in-up" style={{ background: "white", borderColor: "#e5e2d8" }}>
