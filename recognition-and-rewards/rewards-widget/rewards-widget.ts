...
const factory: BlockFactory = (BaseBlockClass, widgetApi: WidgetApi) => {
  return class RewardsWidget extends BaseBlockClass implements BaseBlock {
    constructor() {
      super();
    }
    ...
    async renderBlock(container: HTMLElement): Promise<void> {
      ...
      function rowToEntry(row: { redemptionTitle?: string; redemptionDateTime?: string }): HistoryEntry {
        const title = row.redemptionTitle || "";
        const match = catalog.find(c => c.name === title);
        return { name: title, icon: match?.icon || "ti-gift", pts: match?.pts || 0, date: fmtDate(row.redemptionDateTime || "") };
      }

      async function loadRemoteHistory(): Promise<void> {
        if (!redemptionUrl || !userId) return;
        try {
          const r = await fetch(`${redemptionUrl}?userId=${encodeURIComponent(userId)}`);
          const d = await r.json();
          history = (d.data || []).map(rowToEntry);
          try { localStorage.setItem(historyKey, JSON.stringify(history.slice(0, 50))); } catch {}
          renderHistory();
        } catch {}
      }

      function saveRemoteRedemption(item: CatalogItem, iso: string): void {
        if (!redemptionUrl || !userId) return;
      ...
      // Compute tier
      function renderTierBar(pts: number): string {
...
<div class="${p}-toast" id="toast"></div>`;
      ...
      let catFilter = "all";
      let pendingItem: CatalogItem | null = null;
      let gridIntroUsed = false;
      let checkoutBusy = false; // guards against double-submit while points are persisting

      const ptsDisplay = container.querySelector("#ptsDisplay") as HTMLElement;
      const tierBar = container.querySelector("#tierBar") as HTMLElement;
      const rewardsGrid = container.querySelector("#rewardsGrid") as HTMLElement;
      const histList = container.querySelector("#histList") as HTMLElement;
      const modalBg = container.querySelector("#modalBg") as HTMLElement;
      const modalBox = container.querySelector("#modalBox") as HTMLElement;
      ...
      function renderGrid() {
        ...
        if (!gridIntroUsed) { rewardsGrid.classList.add("intro"); gridIntroUsed = true; }
        else rewardsGrid.classList.remove("intro");
  ...
  ${h.pts ? `<div class="${p}-hpts">-${h.pts.toLocaleString()} pts</div>` : ""}
      ...
      container.querySelectorAll(`.${p}-tab`).forEach(btn => {
        btn.addEventListener("click", () => {
          ...
          (container.querySelector("#view-shop") as HTMLElement).style.display = t === "shop" ? "" : "none";
          (container.querySelector("#view-history") as HTMLElement).style.display = t === "history" ? "" : "none";
      ...
      container.querySelectorAll(`.${p}-cat`).forEach(btn => {
        btn.addEventListener("click", () => {
          catFilter = (btn as HTMLElement).dataset.cat!;
      ...
      function closeCheckout() {
        if (checkoutBusy) return; // don't dismiss mid-transaction
      ...
      function showSuccess(item: CatalogItem) {
        checkoutBusy = false;
      ...
      function bindModal() {
        modalBox.querySelectorAll("[data-act]").forEach(btn => {
          btn.addEventListener("click", async () => {
            ...
            if (act === "cancel" || act === "done") { closeCheckout(); return; }
            if (act === "confirm" && pendingItem) await runRedemption(pendingItem);
          });
        });
      }

      async function runRedemption(item: CatalogItem) {
        showProcessing();
        try {
          // Re-read the live balance to avoid races, then deduct against it.
          let latestPts = currentPts;
          if (userId) {
            const resp = await fetch(`${baseUrl}/users/${userId}`, apiOpts());
            const data = await resp.json();
            latestPts = parseInt(data?.profile?.[pointsField] || "0", 10);
          }
          ...
          if (userId) {
            ...
            if (!resp.ok) throw new Error("Failed to update points");
          }
          ...
          const entry: HistoryEntry = { name: item.name, icon: item.icon, pts: item.pts, date: fmtDate(nowIso) };
      ...
      modalBg.addEventListener("click", (e) => {
        if (e.target === modalBg) closeCheckout();
      });
