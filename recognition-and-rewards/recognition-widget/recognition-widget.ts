    ...
    <div class="${p}-toast" id="toast"></div>
      ...
      recipSearch.addEventListener("input", () => {
        ...
        searchTimeout = setTimeout(async () => {
          try {
            let users: UserItem[] = [];
            const res = await widgetApi.getUserList({ limit: 20, filter: q });
            users = (res.data || []).map((u: { id?: string; firstName?: string; lastName?: string; avatar?: any }) => ({
              id: u.id || "",
          ...
          feed.querySelectorAll(`.${p}-edit-btn`).forEach(btn => {
            btn.addEventListener("click", () => {
              ...
              actionsDiv.querySelector(`.${p}-save-btn`)!.addEventListener("click", async () => {
                ...
                if (!newMsg) return;
                try {
                  await fetch(`${baseUrl}/channels/${channelId}/posts/${postId}`, {
                    ...apiOpts({
                      method: "PUT",
                      body: JSON.stringify({
