...
const STRINGS = {
    en_US: {
        ...
        alignmentAcrossOne: "1 campaign · {n} responses",
        alignmentTip: "Alignment survey average · {n} responses",
        alignmentFloor: "Alignment results appear once {n} people have answered.",
        alignmentPending: "Alignment below reporting threshold",
        alignmentNoResponses: "Alignment survey — no responses yet",
        alignmentSample: "Alignment scores below are sample data, not real survey results.",
        proofLabel: "Proof",
    ...
    de_DE: {
        ...
        alignmentAcrossOne: "1 Kampagne · {n} Antworten",
        alignmentTip: "Durchschnitt der Alignment-Umfrage · {n} Antworten",
        alignmentFloor: "Alignment-Ergebnisse erscheinen, sobald {n} Personen geantwortet haben.",
        alignmentPending: "Alignment unter der Auswertungsgrenze",
        alignmentNoResponses: "Alignment-Umfrage — noch keine Antworten",
        alignmentSample: "Die Alignment-Werte unten sind Beispieldaten, keine echten Umfrageergebnisse.",
        proofLabel: "Nachweis",
    ...
    ar_SA: {
        ...
        alignmentAcrossOne: "حملة واحدة · {n} استجابة",
        alignmentTip: "متوسط استطلاع التوافق · {n} استجابة",
        alignmentFloor: "تظهر نتائج التوافق بعد إجابة {n} أشخاص.",
        alignmentPending: "التوافق دون حد العرض",
        alignmentNoResponses: "استطلاع التوافق — لا توجد استجابات بعد",
        alignmentSample: "درجات التوافق أدناه بيانات نموذجية وليست نتائج استطلاع حقيقية.",
        proofLabel: "إثبات",
...
;// ./manager-tasks-widget.ts
var manager_tasks_widget_awaiter = (undefined && undefined.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
...
function alignmentReportable(r) {
...
const factory = (BaseBlockClass, widgetApi) => {
    return class ManagerTasksWidget extends BaseBlockClass {
        constructor() { super(); }
        renderBlock(container) {
            return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                ...
                // ── Manager-view config ───────────────────────────────────────────
                const teamSource = (this.getAttribute("teamsource") || "reports");
          ...
          .${p}.${p}-limited::-webkit-scrollbar-thumb:hover{background:rgba(${primaryRgb},.55);background-clip:padding-box}` : "";
                ...
                const activityTypeFilters = new Set(); // event type filter (full log)
                const activityPriorities = new Set(); // priority filter (full log)
                ...
                const rankings = new Map(); // campaignId → analytics row
      ...
      `;
                ...
                const dateToEl = container.querySelector(`#${p}-date-to`);
                const dateClearBtn = container.querySelector(`#${p}-date-clear`);
                ...
                if (self._mgrOverlay) {
                    self._mgrOverlay.remove();
                    self._mgrOverlay = undefined;
                }
                if (self._mgrDetail) {
                    self._mgrDetail.remove();
                    self._mgrDetail = undefined;
                }
                if (self._mgrAModal) {
                    self._mgrAModal.remove();
                    self._mgrAModal = undefined;
                }
                if (self._mgrCreate) {
                    self._mgrCreate.remove();
                    self._mgrCreate = undefined;
                }
                if (self._mgrDocClick) {
      ...
      `;
        ...
        <div class="${p}-amodal-card">
          ...
          <div class="${p}-amodal-body" id="${p}-amodal-body-${instId}"></div>
                ...
                attModal.addEventListener("click", e => { if (e.target === attModal)
                    closeAttModal(); });
                aDl.addEventListener("click", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                    if (!dlUrl)
                        return;
                    const name = dlName;
                    try {
          ...
          <pre class="${p}-dbg-body"></pre>`;
                    ...
                    copyBtn.addEventListener("click", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                ...
                function fetchRankingsWith(makeOpts) {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        const url = `${baseUrl}/branch/analytics/campaigns/rankings?orderBy=campaign_title_ASC&limit=100`;
                        dlog("GET campaign rankings", url);
                        const r = yield fetch(url, Object.assign(Object.assign({}, makeOpts()), { headers: Object.assign({ Accept: "*/*" }, (makeOpts().headers || {})) }));
                        const raw = yield r.text();
                ...
                function loadRankings() {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        let rows = null;
                        try {
                            rows = yield fetchRankingsWith(apiOpts);
                        }
                        catch (e) {
                            dlog("rankings via token failed", e.message, "— retrying with session");
                            try {
                                rows = yield fetchRankingsWith(sessionOpts);
                            }
                            catch (e2) {
                                dlog("rankings via session failed", e2.message);
                            }
                        }
                        rankings.clear();
                        for (const r of rows || [])
                            if (r && r.campaignId)
                                rankings.set(r.campaignId, r);
                        ...
                        if (dummyAlignment) {
                            for (const c of campaigns) {
                                ...
                                rankings.set(c.id, Object.assign(Object.assign({}, (live || { campaignId: c.id, campaignTitle: c.title })), { campaignId: c.id, alignmentScore: demo.score, alignmentParticipantsCount: demo.participants, hasAlignmentSurvey: demo.hasSurvey }));
                            }
                ...
                const rankingFor = (id) => rankings.get(id) || null;
                ...
                const campaignTitle = (ref) => { var _a; return ((_a = resolveCampaign(ref)) === null || _a === void 0 ? void 0 : _a.title) || ""; };
                ...
                const userDisplayName = (u) => [u === null || u === void 0 ? void 0 : u.firstName, u === null || u === void 0 ? void 0 : u.lastName].filter(Boolean).join(" ") || (u === null || u === void 0 ? void 0 : u.displayName) || (u === null || u === void 0 ? void 0 : u.userName) || (u === null || u === void 0 ? void 0 : u.id) || "";
                const userAvatarUrl = (u) => { var _a, _b, _c, _d, _e, _f; return ((_b = (_a = u === null || u === void 0 ? void 0 : u.avatar) === null || _a === void 0 ? void 0 : _a.icon) === null || _b === void 0 ? void 0 : _b.url) || ((_d = (_c = u === null || u === void 0 ? void 0 : u.avatar) === null || _c === void 0 ? void 0 : _c.thumb) === null || _d === void 0 ? void 0 : _d.url) || ((_f = (_e = u === null || u === void 0 ? void 0 : u.avatar) === null || _e === void 0 ? void 0 : _e.original) === null || _f === void 0 ? void 0 : _f.url) || ""; };
                ...
                function fetchAllUsersRaw() {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        ...
                        try {
                            ...
                            allUsersRaw = ((d === null || d === void 0 ? void 0 : d.data) || (Array.isArray(d) ? d : []));
                        }
                        catch (_) {
                            allUsersRaw = [];
                        }
                        if (!usersList)
                            usersList = allUsersRaw.map(u => ({ id: u.id, name: userDisplayName(u) }));
                        return allUsersRaw;
                    });
                }
                ...
                function resolveTeam() {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        ...
                        teamMemberSet.clear();
                        teamNote = "";
                        const users = yield fetchAllUsersRaw();
                        ...
                        if (teamSource === "everyone") {
                            ...
                            if (!teamMembers.length)
                                teamNote = tr("noTeamConfigured");
                        }
                        else { // reports — match others' profile.system_manager against my externalID
                            let ext = String((_b = (_a = byId.get(currentUserId)) === null || _a === void 0 ? void 0 : _a.externalID) !== null && _b !== void 0 ? _b : "");
                            if (!ext && currentUserId) {
                                try {
                                    const r = yield fetch(`${baseUrl}/users/${currentUserId}`, apiOpts());
                                    if (r.ok) {
                                        const d = yield r.json();
                                        ext = String((_c = d.externalID) !== null && _c !== void 0 ? _c : "");
                                    }
                                }
                                catch (_) { }
                            }
                            const reports = ext ? users.filter(u => { var _a, _b; return String((_b = (_a = u.profile) === null || _a === void 0 ? void 0 : _a.system_manager) !== null && _b !== void 0 ? _b : "") === ext; }) : [];
                ...
                function displayNameSync(id) {
                    ...
                    if (m)
                        return m.name;
                    const u = (allUsersRaw || []).find(x => x.id === id);
                ...
                function sortTasks(list) {
                    ...
                    const created = (t) => t.createDate ? Date.parse(t.createDate) : 0;
                    const aName = (t) => t.assigneeIds.length ? displayNameSync(t.assigneeIds[0]).toLowerCase() : "~";
                    const arr = list.slice();
                    if (sortBy === "priority")
                        arr.sort((a, b) => rank(a) - rank(b) || due(a) - due(b));
                    else if (sortBy === "assignee")
                        arr.sort((a, b) => aName(a).localeCompare(aName(b)) || due(a) - due(b));
                    else if (sortBy === "created")
                        arr.sort((a, b) => created(b) - created(a));
                    else
                        arr.sort((a, b) => due(a) - due(b)); // "due" default
                    return arr;
                }
                ...
                function wireReassign(task) {
                    ...
                    const foot = root.querySelector(`.${p}-reassign-foot`);
                    ...
                    const apply = () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
              ...
              ${ckIco}
                ...
                function metaCached(id) {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () { if (mediaCache.has(id))
                ...
                function notifyAssigned(userIds, groups, title) {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        if (!notifyOnAssign)
                            return;
                        const send = (ids, text) => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                            if (!ids.length)
                                return;
            ...
            ${items.map((it, i) => {
                ...
                function loadProofItems() {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        if (proofCache)
                            return proofCache;
                        const inScopeBroad = (t) => t.taskType !== "audit-result" &&
                        ...
                        const groups = results.filter((x) => !!x);
                ...
                function proofWithinCompleted(iso) {
                    ...
                    if (isNaN(c))
                        return false;
                    if (assignedFrom && c < Date.parse(`${assignedFrom}T00:00:00`))
                        return false;
                    if (assignedTo && c > Date.parse(`${assignedTo}T23:59:59.999`))
                        return false;
                    return true;
                }
          ...
          <div class="${p}-pg-wrap" id="${p}-pg-wrap"></div>`;
                    const s = proofViewEl.querySelector(`#${p}-pg-search`);
                    const df = proofViewEl.querySelector(`#${p}-pg-date-from`);
                    const dt = proofViewEl.querySelector(`#${p}-pg-date-to`);
                    if (s)
                        s.value = searchQuery;
                    if (df)
                        df.value = assignedFrom;
                    if (dt)
                        dt.value = assignedTo;
                    const syncMainClear = () => { if (dateClearBtn)
                        dateClearBtn.hidden = !(assignedFrom || assignedTo); };
                    let pst;
                    ...
                    dt === null || dt === void 0 ? void 0 : dt.addEventListener("change", () => { assignedTo = dt.value || ""; if (dateToEl)
                        dateToEl.value = assignedTo; syncMainClear(); repaintProofGrid(); });
                }
                function repaintProofGrid() {
                    return manager_tasks_widget_awaiter(this, arguments, void 0, function* (reset = true) {
                        var _a;
                        if (!proofViewEl)
                            return;
                        if (reset)
                            proofLimit = PROOF_PAGE;
                        const wrap = proofViewEl.querySelector(`#${p}-pg-wrap`);
                        if (!wrap)
                            return;
                        ...
                        try {
                            ...
                            if (seq === proofViewSeq)
                                wrap.innerHTML = `<div class="${p}-state"><strong>${tr("failedToLoad")}</strong>${esc(e.message)}</div>`;
                            return;
                        }
                        if (seq !== proofViewSeq)
                            return;
                        ...
                        const ids = new Set();
                        shown.forEach(ph => ids.add(ph.item.id));
                        ...
                        (_a = wrap.querySelector(`#${p}-pg-more`)) === null || _a === void 0 ? void 0 : _a.addEventListener("click", () => { proofLimit += PROOF_STEP; repaintProofGrid(false); });
                    });
                }
                function renderProofView() {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        ...
                        if (!tasksLoaded) {
                            ...
                            yield whenTasksLoaded();
                            if (seq !== proofViewSeq || proofViewEl.style.display === "none")
                                return;
                        }
                ...
                function taskMatches(t, ignoreStore = false) {
                    if (t.taskType === "audit-result")
                        return false; // always hide system tasks
                    if (!inTeam(t))
                        return false; // manager view: team-member dropdown filter
                    if (!ignoreStore && activeInstallFilter !== "all" && t.installationId !== activeInstallFilter)
                        return false;
                ...
                // ── Manager dashboard ──────────────────────────────────────────────
                const isDoneStatus = (t) => t.status === "DONE" || t.status === "done" || t.status === "CLOSED";
                const isOverdue = (t) => !isDoneStatus(t) && !!t.dueDate && new Date(t.dueDate).getTime() < Date.now();
          ...
          <text x="36" y="36" text-anchor="middle" dominant-baseline="central" class="${p}-donut-txt" style="font-size:${fs}px">${Math.round(v)}%</text>
                ...
                function renderCharts() {
                    ...
                    const pCount = (lv) => openTasks.filter(t => t.priority === lv).length;
          ...
          <div class="${p}-dash">
            ...
            ${memberHtml ? `<div class="${p}-dash-members"><div class="${p}-dash-h">${tr("byMember")}</div>${memberHtml}</div>` : ""}
                ...
                function alignmentChip(id) {
                    const r = rankingFor(id);
                    if (!r || !r.hasAlignmentSurvey)
                        return "";
          ...
          <div class="${p}-dash">
            ...
            <div class="${p}-dash-members">
              ...
              ${empty.length ? `<div class="${p}-camp-emptyh">${tr("campaignsNoTasks")}</div>${empty.map(r => row(r, true)).join("")}` : ""}
                ...
                function describeRrule(desc) {
                    const m = RRULE_REGEX.exec(desc);
                    if (!m)
                        return tr("cadDaily");
                    const kv = {};
                    m[0].replace(/^\[rrule:\s*/i, "").replace(/\]$/, "").split(";").forEach(part => { const i = part.indexOf("="); if (i > 0)
                        kv[part.slice(0, i).trim()] = part.slice(i + 1).trim(); });
                    const DN = { SU: "Sunday", MO: "Monday", TU: "Tuesday", WE: "Wednesday", TH: "Thursday", FR: "Friday", SA: "Saturday" };
                    const f = (kv.f || "DAILY").toUpperCase();
                    if (f === "WEEKLY") {
                        const days = (kv.d || "").split(",").map(s => s.trim().toUpperCase()).filter(d => DN[d]);
                        if (!days.length)
                            return tr("cadDaily");
                        if (days.length === 5 && ["MO", "TU", "WE", "TH", "FR"].every(d => days.indexOf(d) !== -1))
                            return tr("cadWeekdays");
                        if (days.length === 1)
                            return tr("cadWeeklyEvery").replace("{d}", DN[days[0]]);
                        return tr("cadWeeklyDays").replace("{d}", days.map(d => DN[d] + "s").join(", "));
                    }
                    if (f === "MONTHLY") {
                        const n = parseInt(kv.dom || "", 10);
                        if (!isNaN(n))
                            return tr("cadMonthlyDom").replace("{n}", ordinal(n));
                        return tr("cadMonthly");
                    }
                    return tr("cadDaily");
                }
                ...
                function activityScope() {
                    ...
                    const priF = activityFull ? activityPriorities : new Set();
                    ...
                    const withinDate = (when) => (!fromMs || when >= fromMs) && (!toMs || when <= toMs);
                ...
                function buildActivityEvents() {
                    ...
                    const hasProofFor = (taskId, author, when) => {
                        const arr = proofByTask.get(taskId);
                        if (!arr || !arr.length)
                            return false;
                    ...
                    activityComments.forEach((comments, taskId) => {
                        ...
                        for (const c of comments) {
                            if (isProofComment(c))
                                continue; // folded into the completion event
                            const txt = commentPlain(c);
                            if (!txt)
                                continue;
                ...
                function loadActivityComments(maxTasks) {
                    return manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        ...
                        const touched = (t) => Math.max(t.createDate ? Date.parse(t.createDate) : 0, t.updateDate ? Date.parse(t.updateDate) : 0);
                        const need = allTasks
                ...
                function setActivityFull(on) {
                    ...
                    if (on) {
                        ...
                        activeTypeFilters.forEach(v => activityTypeFilters.add(v));
                        activityPriorities.clear();
                        prioritySet.forEach(v => activityPriorities.add(v));
                        activityFrom = "";
                        activityTo = ""; // event-date range starts unset
                ...
                function renderActivity() {
                    ...
                    for (const t of allTasks) {
                        ...
                        if ((activityInstallFilter === "all" || t.installationId === activityInstallFilter) && t.taskType)
                            actTypeSet.add(t.taskType);
                    }
          ...
          ${footer}`;
                    ...
                    if (activityFull) {
                        ...
                        const typeDd = activityEl.querySelector(`#${p}-act-type-dd`);
                        if (typeDd)
                            makeDropdown({
                        ...
                        const prioDd = activityEl.querySelector(`#${p}-act-prio-dd`);
                        if (prioDd)
                            makeDropdown({
                        ...
                        const actFrom = activityEl.querySelector(`#${p}-act-date-from`);
                        const actTo = activityEl.querySelector(`#${p}-act-date-to`);
                        actFrom === null || actFrom === void 0 ? void 0 : actFrom.addEventListener("change", () => { activityFrom = actFrom.value || ""; renderActivity(); });
                        actTo === null || actTo === void 0 ? void 0 : actTo.addEventListener("change", () => { activityTo = actTo.value || ""; renderActivity(); });
                ...
                function renderStoreTabs() {
                    ...
                    for (const t of allTasks) {
                        if (t.taskType === "audit-result")
                            continue;
                        if (!titleOf.has(t.installationId))
                            titleOf.set(t.installationId, t.installationTitle || t.installationId);
                    }
                    if (titleOf.size <= 1) {
                    ...
                    let total = 0;
                    for (const t of allTasks) {
                        if (taskMatches(t, true)) {
                            count.set(t.installationId, (count.get(t.installationId) || 0) + 1);
                            total++;
                        }
                    }
          ...
          ${(isCrit || (task.priority && task.priority !== "Priority_3")) ? `<span class="${p}-prio-badge${isCrit ? " crit" : ""}" style="color:${prioCol};border-color:${prioCol}">${prioLbl}</span>` : ""}`;
                    ...
                    const nameForUid = (id) => { const m = teamMembers.find(x => x.id === id); if (m)
                        return m.name; const u = (allUsersRaw || []).find(x => x.id === id); return u ? userDisplayName(u) : id; };
                    let assigneeHtml = "";
        ...
        `;
                    ...
                    if (enableComments) {
                        ...
                        cFile === null || cFile === void 0 ? void 0 : cFile.addEventListener("change", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                        ...
                        const submit = () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                    ...
                    if (attAdd && attInput) {
                        ...
                        attInput.addEventListener("change", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                ...
                detailToggle.addEventListener("click", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                    if (!detailTask)
                        return;
                ...
                const refreshViews = () => { renderList(); if (proofViewEl && proofViewEl.style.display !== "none")
                    renderProofView(); };
                if (searchInput) {
                    let st;
                    searchInput.addEventListener("input", () => { clearTimeout(st); st = setTimeout(() => { searchQuery = searchInput.value.trim(); refreshViews(); }, 180); });
                }
                const syncDateClear = () => { if (dateClearBtn)
                    dateClearBtn.hidden = !(assignedFrom || assignedTo); };
                dateFromEl === null || dateFromEl === void 0 ? void 0 : dateFromEl.addEventListener("change", () => { assignedFrom = dateFromEl.value || ""; syncDateClear(); refreshViews(); });
                dateToEl === null || dateToEl === void 0 ? void 0 : dateToEl.addEventListener("change", () => { assignedTo = dateToEl.value || ""; syncDateClear(); refreshViews(); });
            ...
            <div class="${p}-create-foot">
              ...
              <button type="button" class="${p}-btn-save" id="${p}-c-save">${tr("createTask")}</button>
                        ...
                        const cSelUsers = new Set(), cSelGroups = new Set();
                        ...
                        const cGroups = () => { const o = []; groupMap.forEach((name, id) => o.push({ id, name })); return o.sort((a, b) => a.name.localeCompare(b.name)); };
                        const cLabel = () => {
                            const names = [...[...cSelGroups].map(id => groupMap.get(id) || id), ...[...cSelUsers].map(id => { var _a; return ((_a = (usersList || []).find(u => u.id === id)) === null || _a === void 0 ? void 0 : _a.name) || id; })];
                        ...
                        const cRender = (q) => {
                            ...
                            let html = "";
                            if (groups.length)
                                html += `<div class="${p}-reassign-h">${tr("groups")}</div>` + groups.map(g => `<div class="${p}-reassign-opt${cSelGroups.has(g.id) ? " sel" : ""}" data-type="group" data-id="${esc(g.id)}">${cgIco}<span>${esc(g.name)}</span>${cckIco}</div>`).join("");
                            ...
                            cResults.querySelectorAll(`.${p}-reassign-opt`).forEach((o) => o.addEventListener("click", () => {
                                const el = o;
                                const id = el.dataset.id;
                                const set = el.dataset.type === "group" ? cSelGroups : cSelUsers;
                                if (set.has(id))
                                    set.delete(id);
                                else
                                    set.add(id);
                        ...
                        cBtn.addEventListener("click", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                            if (cPop.style.display !== "none") {
                        ...
                        $("c-save").addEventListener("click", () => manager_tasks_widget_awaiter(this, void 0, void 0, function* () {
                            ...
                            try {
                                ...
                                if (cSelUsers.size)
                                    body.assigneeIds = [...cSelUsers];
                                if (cSelGroups.size)
                                    body.groupIds = [...cSelGroups];
        ...
        disconnectedCallback() {
            ...
            if (self._mgrVV && window.visualViewport) {
                ...
                window.visualViewport.removeEventListener("scroll", self._mgrVV);
