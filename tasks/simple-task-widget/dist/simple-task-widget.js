    ...
    return class SimpleTaskWidget extends BaseBlockClass {
        ...
        async renderBlock(container) {
          ...
          .${p}.${p}-limited::-webkit-scrollbar-thumb:hover{background:rgba(${primaryRgb},.55);background-clip:padding-box}` : "";
            ...
            if (self._stwProof) {
                self._stwProof.remove();
                self._stwProof = undefined;
            }
            if (self._stwDocKey) {
          ...
          <div class="${p}-row-main">
            ...
            async function load() {
                ...
                tasks = await Promise.all(refs.map(async (r) => {
                    const base = { id: r.taskId, installId: r.installId, title: "", description: "", status: "", dueDate: null, priority: "Priority_3", taskType: null, isRecurring: false, createDate: null, groupIds: [], assigneeIds: [], attachmentIds: [], ok: false };
                    try {
