    ...
    return class MyTasksWidget extends BaseBlockClass {
      ...
      `;
        ...
        <div class="${p}-amodal-card">
          ...
          <pre class="${p}-dbg-body"></pre>`;
                ...
                function wireReassign(task) {
                    ...
                    const renderResults = (q) => {
                        ...
                        let html = "";
                        if (groups.length) {
                            html += `<div class="${p}-reassign-h">${tr("groups")}</div>` + groups.map(g => `<div class="${p}-reassign-opt" data-type="group" data-id="${esc(g.id)}">${gIco}<span>${esc(g.name)}</span></div>`).join("");
                        }
          ...
          ${(pa === null || pa === void 0 ? void 0 : pa.notes) ? `<div class="${p}-detail-desc-label">${tr("notes")}</div><div class="${p}-detail-desc" style="margin-bottom:18px">${esc(pa.notes)}</div>` : ""}
