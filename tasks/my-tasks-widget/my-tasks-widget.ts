        ...
        if(sysTask&&sysTask.attachmentIds&&sysTask.attachmentIds.length){
          ...
          Promise.all(ids.map(mediaMeta)).then(metas=>{
            ...
            grid.innerHTML=ids.map((_id,i)=>{
              ...
              const size=m?.size?`<span class="${p}-att-size">${humanSize(m.size)}</span>`:"";
              return `<div class="${p}-att-tile"><a class="${p}-att-link" href="${esc(m?.thumbnail?.url||"#")}" target="_blank" rel="noopener">${thumb}<span class="${p}-att-meta"><span class="${p}-att-name">${name}</span>${size}</span></a></div>`;
            }).join("");
          });
        }
            ...
            try{
              ...
              for(const result of perList){
                ...
                for(const t of arr){
                  ...
                  if(taskType==="recur-template"){ seen.delete(t.id); continue; }
