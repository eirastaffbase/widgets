    ...
    async renderBlock(container: any) {
      ...
      function resolveAttachments(html:string):string{
        return html.replace(ATT_TOKEN,(_m,id)=>{
          ...
          const href=meta?.thumbnail?.url||"";
          const isImg=/^image\//.test(meta?.mimeType||"");
          if(isImg&&meta?.thumbnail?.url){
      ...
      function renderAuditTabs(){
        ...
        auditTabsEl.querySelectorAll(`.${p}-audit-tab`).forEach(btn=>{
          btn.addEventListener("click",()=>{
            ...
            showOtherAuditTasks=false; showAuditChart=false;
      ...
      function renderAuditContent(){
        ...
        const summaryHtml=pa?`
          <div class="${p}-audit-card ${passClass}">
            ...
            ${(()=>{
              ...
              const bars=cats.map((name:string)=>{
        ...
        const catToggle=listWrap.querySelector(`#${p}-cat-toggle`) as HTMLElement|null;
        if(catToggle) catToggle.addEventListener("click",()=>{ showAuditChart=!showAuditChart; renderAuditContent(); });
      ...
      function renderDetailContent(task:Task){
        ...
        if(enableComments){
          ...
          cInput?.addEventListener("blur",()=>{ setTimeout(()=>{ if(!hasContent()) cBar?.classList.remove("show"); },200); });
