...
import { linkifyEscaped, shortenUrls, AUTOLINK_CSS, internalHost, installLinkHandler } from "../shared/linkify";
    ...
    async renderBlock(container: any) {
          ...
          bodyHtml = `<div class="${p}-cal-cols" style="grid-template-columns:repeat(${calDays},1fr)">` + days.map(d => {
            ...
            const evs = occ.length ? occ.map(s =>
              `<div class="${p}-ev" data-id="${esc(s.id)}">
                ...
                ${s.description ? `<div class="${p}-ev-desc">${shortenUrls(esc(s.description),selfHost)}</div>` : ""}
              </div>`
      ...
      function openDetail(s: Schedule) {
        ...
        detailBody.innerHTML = `
          ...
          ${s.description ? `<div class="${p}-detail-desc-label">${tr("description")}</div><div class="${p}-detail-desc">${linkifyEscaped(esc(s.description), selfHost)}</div>` : ""}`;
