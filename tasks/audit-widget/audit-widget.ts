    ...
    async renderBlock(container: any) {
      ...
      function parseRows(rows: any[][]): Question[] {
        ...
        for(let i=0;i<Math.min(4,rows.length);i++){
          if(rows[i].some((c:any)=>/question|text|id/i.test(String(c||"")))){hIdx=i;break;}
        }
        ...
        const iId=col("question id","id"); const iCat=col("category","cat"); const iText=col("question text","text","question");
        const iType=col("type"); const iPts=col("pts","point"); const iCrit=col("critical");
        const iTask=col("task flag","auto-task","task"); const iTitle=col("task title","title");
        const iRole=col("task role","role"); const iPrio=col("priority"); const iDue=col("due");
        const iActive=13;
        const out:Question[]=[];
      ...
      function renderSetup(){
        const opts=installations.map(i=>
          `<option value="${esc(i.id)}"${i.id===selectedInstId?" selected":""}>${esc(i.title)}</option>`
        ).join("");
        contentEl.innerHTML=`
          <div class="${p}-card">
        ...
        const storeSel=(contentEl.querySelector(`#${p}-store`) as HTMLSelectElement);
        if(selectedInstId) storeSel.value=selectedInstId;
        contentEl.querySelector(`#${p}-begin`)!.addEventListener("click",()=>{
          selectedInstId=(contentEl.querySelector(`#${p}-store`)  as HTMLSelectElement).value;
